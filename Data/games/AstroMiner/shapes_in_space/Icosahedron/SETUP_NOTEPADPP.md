...
This guide shows you how to compile and run your Icosahedron game directly from Notepad++ using keyboard shortcuts.
...
ENDIF
IF $(EXITCODE) == 0 THEN
    IF EXIST "bin\Icosahedron.exe" THEN
        cmd /c "start bin\Icosahedron.exe"
    ENDIF
ENDIF
```

3. Click **Save...**
4. Name it: **"Build and Run Icosahedron"**
5. Click **Save**

### Step 4: Assign Keyboard Shortcuts

1. Go to **Plugins** → **NppExec** → **Advanced Options...**
2. Under **Menu Item**, select **"Build Icosahedron"**
3. Click **Add/Modify**
4. Check **"Place to Macros submenu"** (optional, or leave unchecked for main menu)
5. Click **OK**
6. Repeat for **"Build and Run Icosahedron"**
7. Go to **Settings** → **Shortcut Mapper...**
8. Click **Plugin commands** tab
9. Find your scripts (they'll be named like "NppExec: Build Icosahedron")
...
2. Paste this for building (keeps terminal open):
   ```
   cmd /c "cd /d C:\Dev Projects\Icosahedron && build_simple.bat && pause"
   ```
3. Click **Save...** and name it "Build"
4. For building and running (with console open):
   ```
   cmd /c "cd /d C:\Dev Projects\Icosahedron && build_simple.bat && if exist bin\Icosahedron.exe (bin\Icosahedron.exe && pause)"
   ```
