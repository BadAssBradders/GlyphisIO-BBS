@echo off
REM Build script for Torus Raylib project (Release configuration)
REM Edit your code in Notepad++, then run this script to build

echo Building Torus project (Release)...
echo.
...
"%MSBUILD_PATH%" Torus.sln /p:Configuration=Release /p:Platform=x64 /m
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo.
    echo Executable location: bin\x64\Release\Torus.exe
    echo.
    echo To run the game, execute: bin\x64\Release\Torus.exe
) else (
    echo.
